...
public class Metrics {
    ...
    public static Metric put(@Context HttpServletRequest request, InputStream input, @DefaultValue("xxx") @QueryParam("token") String token) throws JAXBException, DBException {
    ...
    @GET
    @Path("remove/{id}")
    public static String remove(@PathParam("id") int id, @DefaultValue("xxx") @QueryParam("token") String token) throws JAXBException, DBException, Exception {
        if (!token.equals("4c021c79-6862-4337-bc24-923320e41354")) {
            ResponseBuilderImpl responseBuilder = new ResponseBuilderImpl();
            responseBuilder.status(javax.ws.rs.core.Response.Status.FORBIDDEN);
            responseBuilder.entity("Token not recognized");
        ...
        if (m.id == 0) {
            return "Metric(" + id + ") does not exist";
        }
        delete(m);
        return "OK";
    }
    ...
    public static REList<MetricValue> getValues(
            ...
            @DefaultValue("xxx") @QueryParam("token") String token
        ...
        if (start >= 0) {
            tstart = new Timestamp(0);
        }
        if (end > 0) {
            tend = new Timestamp(end);
        }
        Deployment dep = null;
        if (!deploymentID.equals("-1")) {
            dep = getDeploymentById(deploymentID);
        }
